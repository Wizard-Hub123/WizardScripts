...
-- SPEED HUB STYLE PURCHASE (Saktong-sakto sa Logs mo)
local function fastPurchase(shopType, data)
    ...
    pcall(function()
        if shopType == "SeedShop" then
            ...
            -- Logic para sa Gear (Base sa log mo: "GearShop", "Item Name")
...
-- ANTI-AFK (Stay Online)
local VirtualUser = game:service'VirtualUser'
game:service'Players'.LocalPlayer.Idled:connect(function()
    VirtualUser:CaptureController()
    VirtualUser:ClickButton2(Vector2.new())
end)local Rayfield = loadstring(game:HttpGet('https://sirius.menu/rayfield'))()
local Window = Rayfield:CreateWindow({
    Name = "Wizard Hub | v3.0",
    LoadingTitle = "Loading...",
    ConfigurationSaving = {Enabled = false}
...
SeedTab:CreateToggle({
    ...
    pcall(function()
        ...
        if shopType == "GearShop" then
            remote:InvokeServer(unpack(args))
g.n, Callback = function(v) _G["G"..i] = v task.spawn(function() while _G["G"..i] do fastPurchase("GearShop", g) task.wait(0.05) end end) end}) end
